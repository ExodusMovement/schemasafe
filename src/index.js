...
const compile = (schema, root, opts, scope, basePathRoot) => {
  ...
  const visit = (allErrors, includeErrors, history, current, node, schemaPath) => {
    ...
    const error = (msg, schemaSubPath, prop = current) => {
      /* c8 ignore next */
      if (!Array.isArray(schemaSubPath)) throw new Error('Unreachable: no schemaSubPath')
      if (includeErrors === true) {
        const schemaPathFull = [...schemaPath, ...schemaSubPath]
        const errorObj = { message: msg, schemaPath: functions.toPointer(schemaPathFull) }
    ...
    if (definitelyPresent) {
      ...
      if (booleanRequired) {
        if (node.required === true) {
          if (!defaultIsPresent) error('is required', ['required'])
    ...
    if (node.$ref) {
      ...
      if (sub || sub === false) {
        ...
        errorIf('!%s(%s)', [n, name], 'referenced schema does not match', ['$ref'])
      } else {
    ...
    const checkNumbers = () => {
      const applyMinMax = (value, operator, message, schemaSubPath) => {
        enforce(Number.isFinite(value), 'Invalid minimum or maximum:', value)
        errorIf('!(%d %c %s)', [value, operator, name], message, schemaSubPath)
      }

      if (Number.isFinite(node.exclusiveMinimum)) {
        applyMinMax(node.exclusiveMinimum, '<', 'is less than exclusiveMinimum', ['exclusiveMinimum'])
        consume('exclusiveMinimum', 'finite')
      } else if (node.minimum !== undefined) {
        applyMinMax(node.minimum, node.exclusiveMinimum ? '<' : '<=', 'is less than minimum', ['minimum'])
      ...
      if (Number.isFinite(node.exclusiveMaximum)) {
        applyMinMax(node.exclusiveMaximum, '>', 'is more than exclusiveMaximum', ['exclusiveMaximum'])
        consume('exclusiveMaximum', 'finite')
      } else if (node.maximum !== undefined) {
        applyMinMax(node.maximum, node.exclusiveMaximum ? '>' : '>=', 'is more than maximum', ['maximum'])
      ...
      if (node[multipleOf] !== undefined) {
        ...
        errorIf('!isMultipleOf(%s, %d)', [name, node[multipleOf]], 'has a remainder', ['isMultipleOf'])
    ...
    const checkStrings = () => {
      if (node.maxLength !== undefined) {
        ...
        errorIf('stringLength(%s) > %d', [name, node.maxLength], 'has longer length than allowed', ['maxLength'])
      ...
      if (node.minLength !== undefined) {
        ...
        scope.stringLength = functions.stringLength
        errorIf('stringLength(%s) < %d', [name, node.minLength], 'has less length than allowed', ['minLength'])
      ...
      if (node.format && functions.hasOwn(fmts, node.format)) {
        ...
        if (formatImpl instanceof RegExp || typeof formatImpl === 'function') {
          ...
          if (formatImpl instanceof RegExp) {
            ...
            errorIf('!%s.test(%s)', [n, name], `must be ${node.format} format`, ['format'])
          } else {
            errorIf('!%s(%s)', [n, name], `must be ${node.format} format`, ['format'])
          }
        } else {
      ...
      if (node.pattern) {
        ...
        if (!noopRegExps.has(node.pattern)) {
          ...
          errorIf('!%s.test(%s)', [p, name], 'pattern mismatch', ['pattern'])
        }
    ...
    const checkArrays = () => {
      if (node.maxItems !== undefined) {
        ...
        errorIf('%s.length > %d', [name, node.maxItems], 'has more items than allowed', ['maxItems'])
      ...
      if (node.minItems !== undefined) {
        ...
        errorIf('%s.length < %d', [name, node.minItems], 'has less items than allowed', ['minItems'])
      ...
      if (!Array.isArray(node.items)) {
        ...
        if (removeAdditional) {
          ...
          errorIf('%s.length > %d', [name, limit], 'has additional items', ['additionalItems'])
        }
      ...
      if (node.contains || node.contains === false) {
        ...
        if (Number.isFinite(node.minContains)) {
          errorIf('%s < %d', [passes, node.minContains], 'array contains too few matching items', ['minContains'])
        ...
        if (Number.isFinite(node.maxContains)) {
          errorIf('%s > %d', [passes, node.maxContains], 'array contains too many matching items', ['maxContains'])
      ...
      if (node.uniqueItems === true) {
        ...
        scope.deepEqual = functions.deepEqual
        errorIf('!unique(%s)', [name], 'must be unique', ['uniqueItems'])
    ...
    const checkObjects = () => {
      if (node.maxProperties !== undefined) {
        ...
        errorIf('Object.keys(%s).length > %d', [name, node.maxProperties], 'too many properties', ['maxProperties'])
        consume('maxProperties', 'natural')
      }

      if (node.minProperties !== undefined) {
        enforce(Number.isFinite(node.minProperties), 'Invalid minProperties:', node.minProperties)
        errorIf('Object.keys(%s).length < %d', [name, node.minProperties], 'too few properties', ['minProperties'])
      ...
      if (Array.isArray(node.required)) {
        for (const req of node.required) {
          ...
          errorIf('!(%s)', [present(prop)], 'is required', ['required'], prop)
        }
      ...
      if (node[dependencies]) {
        for (const key of Object.keys(node[dependencies])) {
          ...
          if (Array.isArray(deps)) {
            ...
            errorIf('%s && !(%s)', [present(item), condition], 'dependencies not set', [dependencies, key])
          ...
          fun.block('if (%s) {', [additionalProp], '}', () => {
            if (node.additionalProperties === false) {
              if (removeAdditional) {
                ...
                error('is an additional property', ['additionalProperties'], currPropVar(key))
              }
            } else {
              const sub = currPropVar(key, true) // always own property, from Object.keys
    ...
    const checkConst = () => {
      if (node.const !== undefined) {
        ...
        errorIf('!%s', [compare(node.const)], 'must be const value', ['const'])
        ...
        const compare = makeCompare(name, complex)
        errorIf('!(%s)', [safeor(...node.enum.map(compare))], 'must be an enum value', ['enum'])
        consume('enum', 'array')
        return true
      }
      return false
    ...
    const checkGeneric = () => {
      if (node.not || node.not === false) {
        ...
        error('negative schema matches', ['not'])
      ...
      if (node.oneOf !== undefined) {
        ...
        errorIf('%s !== 1', [passes], 'no (or more than one) schemas match', ['oneOf'])
        consume('oneOf', 'array')
      }
    }
    ...
    if (needTypeValidation) {
      ...
      errorIf('!(%s)', [typeValidate], 'is the wrong type', ['type'])
    }
