...
{
  "$defs": {
    "const_not_anchor": { "const": { "$anchor": "not_a_real_anchor" } }
  },
  "if": { "const": "skip not_a_real_anchor" },
  "then": true,
  "else": { "$ref": "#/$defs/const_not_anchor" }
}
```
...
const hasOwn = Function.prototype.call.bind(Object.prototype.hasOwnProperty);
const ref1 = function validate(data) {
  if (!(typeof data === "object" && data && !Array.isArray(data) && Object.keys(data).length === 1 && hasOwn(data, "$anchor") && data["$anchor"] === "not_a_real_anchor")) return false
  return true
};
const ref0 = function validate(data) {
  const sub0 = (() => {
